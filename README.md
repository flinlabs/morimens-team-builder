...
> **Why steps 2–3?** `public/assets/` is **gitignored** and rebuilt from source, so a fresh clone won't have the art until you sync. Most of `db/*.json` is regenerated by the sync as well; `db/bis.json` and `db/gameplay-math.json` are the exceptions and stay committed, since they are editorial rather than fetched. The `prebuild` hook runs sync and assets automatically before `npm run build`, so production builds self-populate.
...
db/                      # reference data (sync-generated; bis.json + gameplay-math.json committed)
