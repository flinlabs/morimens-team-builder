...
import type { Realm, EnlightenSlot, CharacterAssignment, UserRoster } from "@/lib/types";
...
function AwakenerCard({
  ...
  const floor = item.viabilityFloor ?? "E0";
  const recommended = new Set<EnlightenSlot>([
    floor,
    ...(item.enlightenBreakpoints ?? []),
  ]);
  return (
    <div
      onClick={onToggle}
    ...
    >
      ...
      {/* Enlighten ladder — only when owned.
          Each rung is a button rather than a dropdown so the recommended stops
          are visible without opening anything: the comfort floor and every
          recorded breakpoint are outlined, the current level is filled, and a
          rung below the floor reads as a warning. That is the same data the
          Meta tab ranks on, shown where the player actually sets it. */}
      {owned && (
        <div
          className="flex items-center gap-1 border-t border-[var(--border)] bg-[var(--bg-2)] px-2 py-1.5"
        ...
        >
          ...
          <div className="ml-auto flex items-center gap-0.5">
            {ENLIGHTEN.map((slot) => {
              const isCurrent = slot === enlighten;
              const isRecommended = recommended.has(slot);
              const isFloor = slot === floor;
              const belowFloor = ENLIGHTEN.indexOf(enlighten) < ENLIGHTEN.indexOf(floor);
              return (
                <button
                  key={slot}
                  onClick={() => onEnlighten(slot)}
                  title={
                    isFloor
                      ? `${slot} — comfort floor, worth fielding from here`
                      : isRecommended
                        ? `${slot} — recommended stopping point`
                        : slot
                  }
                  className={`rounded px-1 py-0.5 text-[10px] font-semibold leading-none transition ${
                    isCurrent
                      ...
                      : isRecommended
                        ? "border border-[var(--gold)]/60 text-[var(--gold-bright)] hover:bg-[var(--gold)]/15"
