...
export default function TeamScore({
  ...
  return (
    <span ref={anchorRef} className="inline-flex items-center gap-1.5">
      <span
      ...
      >
        <div className="font-title mb-1 text-[11px] uppercase tracking-wider text-[var(--gold-bright)]">
          Team score {score.toFixed(2)}
        </div>

        <p className="mb-2 text-[11px] leading-relaxed text-[var(--text-dim)]">
          This is what the generator ranks by — the same number, not a recalculation. For
          scale: a random four lands around 0.3–0.5, the curated meta comps score about
          1.05 with everyone at E0 and about 1.3 fully invested. It is a relative signal
          for comparing lineups, not a rating worth optimising.
        </p>

        {partial && (
          <p className="mb-2 rounded border border-[var(--realm-caro)]/40 bg-[var(--realm-caro)]/5 p-1.5 text-[11px] text-[var(--realm-caro)]">
            Only {memberCount} slot{memberCount === 1 ? "" : "s"} filled. Investment is an
            average, so a partial team is not comparable with a full one.
          </p>
        )}
        ...
        <p className="mt-2 border-t border-[var(--border)] pt-2 text-[10px] leading-relaxed text-[var(--text-dim)]">
          Scoring only sees what the annotations record. If a team you know is strong
          scores poorly, the likely cause is a missing pairing or role tag rather than the
          team being wrong — that is worth reporting.
        </p>
      </Popover>
    </span>
  );
